...
// 🔍 ADICIONE ESTE DEBUG TEMPORÁRIO LOGO APÓS O CARD P/L

{/* 🔍 DEBUG P/L - REMOVER DEPOIS */}
<div style={{
  ...
  <h4 style={{ margin: '0 0 12px 0', color: '#856404' }}>🔍 DEBUG P/L - {ticker}</h4>
  
  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(250px, 1fr))', gap: '12px' }}>
    
    {/* Estado dos Hooks */}
    <div style={{ backgroundColor: '#fff', padding: '8px', borderRadius: '4px' }}>
      <strong>📊 Estado dos Hooks:</strong>
      <br/>• loadingHGBrasil: {loadingHGBrasil ? '⏳ SIM' : '✅ NÃO'}
      <br/>• loadingYahoo: {loadingYahoo ? '⏳ SIM' : '✅ NÃO'}
      <br/>• dadosLoading: {dadosLoading ? '⏳ SIM' : '✅ NÃO'}
      <br/>• isMobile: {isMobile ? '📱 SIM' : '🖥️ NÃO'}
    </div>

    {/* Dados HG Brasil */}
    <div style={{ backgroundColor: '#fff', padding: '8px', borderRadius: '4px' }}>
      <strong>🇧🇷 HG Brasil:</strong>
      <br/>• dadosHGBrasil existe: {dadosHGBrasil ? '✅ SIM' : '❌ NÃO'}
      <br/>• dadosHGBrasil.pl: {dadosHGBrasil?.pl || 'undefined'}
      <br/>• dadosHGBrasil.fonte: {dadosHGBrasil?.fonte || 'undefined'}
      <br/>• errorHGBrasil: {errorHGBrasil || 'nenhum'}
    </div>

    {/* Dados Yahoo */}
    <div style={{ backgroundColor: '#fff', padding: '8px', borderRadius: '4px' }}>
      <strong>🌍 Yahoo Finance:</strong>
      <br/>• dadosYahoo existe: {dadosYahoo ? '✅ SIM' : '❌ NÃO'}
      <br/>• dadosYahoo.pl: {dadosYahoo?.pl || 'undefined'}
      <br/>• dadosYahoo.fonte: {dadosYahoo?.fonte || 'undefined'}
      <br/>• errorYahoo: {errorYahoo || 'nenhum'}
    </div>

    {/* Dados BRAPI */}
    <div style={{ backgroundColor: '#fff', padding: '8px', borderRadius: '4px' }}>
      <strong>📈 BRAPI:</strong>
      <br/>• dadosFinanceiros existe: {dadosFinanceiros ? '✅ SIM' : '❌ NÃO'}
      <br/>• dadosFinanceiros.pl: {dadosFinanceiros?.pl || 'undefined'}
      <br/>• dadosError: {dadosError || 'nenhum'}
      <br/>• ultimaAtualizacao: {ultimaAtualizacao || 'undefined'}
    </div>

    {/* Tipo do Ticker */}
    <div style={{ backgroundColor: '#fff', padding: '8px', borderRadius: '4px' }}>
      <strong>🏷️ Tipo do Ticker:</strong>
      <br/>• ticker: {ticker}
      <br/>• é ação brasileira: {(ticker.match(/\d$/) && !ticker.includes('11')) ? '🇧🇷 SIM' : '❌ NÃO'}
      <br/>• é FII: {ticker.includes('11') ? '🏢 SIM' : '❌ NÃO'}
      <br/>• é internacional: {!ticker.match(/\d$/) ? '🌍 SIM' : '❌ NÃO'}
    </div>

    {/* Teste de Prioridade */}
    <div style={{ backgroundColor: '#fff', padding: '8px', borderRadius: '4px' }}>
      <strong>🎯 Teste de Prioridade:</strong>
      <br/>• HG válido: {(dadosHGBrasil?.pl && dadosHGBrasil.pl > 0 && dadosHGBrasil.pl < 1000) ? '✅ SIM' : '❌ NÃO'}
      <br/>• BRAPI válido: {(dadosFinanceiros?.pl && dadosFinanceiros.pl > 0 && dadosFinanceiros.pl < 1000) ? '✅ SIM' : '❌ NÃO'}
      <br/>• Yahoo válido: {(dadosYahoo?.pl && dadosYahoo.pl > 0 && dadosYahoo.pl < 999) ? '✅ SIM' : '❌ NÃO'}
    </div>
  </div>

  {/* Dados Raw */}
  <div style={{ marginTop: '12px', backgroundColor: '#f8f9fa', padding: '8px', borderRadius: '4px' }}>
    <strong>📋 Dados Raw (JSON):</strong>
    <br/>• HG Brasil: {JSON.stringify(dadosHGBrasil, null, 2).substring(0, 200)}...
    <br/>• Yahoo: {JSON.stringify(dadosYahoo, null, 2).substring(0, 200)}...
    <br/>• BRAPI: {JSON.stringify(dadosFinanceiros, null, 2).substring(0, 200)}...
  </div>
  ...
  <div style={{ marginTop: '12px', textAlign: 'center' }}>
    <button
      onClick={() => {
        ...
        // Se você tiver funções de refetch dos outros hooks, adicione aqui
