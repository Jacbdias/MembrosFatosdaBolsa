    ...
    <div style={{ 
              ...
              <div>
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    ✅ Conversão automática para formato internacional
                  </div>
                  <input
                    type="file"
                    accept=".csv"
      ...
      <div style={{
        ...
        <h3 style={{ fontSize: '18px', fontWeight: '700', margin: '0 0 8px 0' }}>
          💱 Sistema de Preço Teto BDR + 📄 Importação CSV com Suporte Brasileiro
        </h3>
        <p style={{ fontSize: '14px', margin: '0', opacity: 0.9 }}>
          Para carteiras internacionais, você pode definir preços teto tanto em USD quanto em BRL (para BDRs). 
          Agora também é possível importar múltiplos ativos via CSV! ✅ Aceita formato brasileiro automaticamente (separador ";" e preços "R$ X,XX").
        </p>
      </div>
    </div>
  );
      ...
      // Parse usando PapaParse (mais robusto)
      ...
      const resultado = Papa.parse(text, {
        ...
        skipEmptyLines: true,
        delimitersToGuess: [',', '\t', '|', ';'],
        transformHeader: (header) => header.trim() // Limpar espaços dos headers
      });
      
      console.log('📄 Headers encontrados:', resultado.meta.fields);
      console.log('📄 Linhas processadas:', resultado.data.length);
      console.log('❌ Erros de parse:', resultado.errors.length);
      ...
      resultado.data.forEach((linha, index) => {
        ...
        if (!linha.ticker || linha.ticker.toString().trim() === '') {
          errosLinha.push('Ticker obrigatório');
        }
        if (!linha.setor || linha.setor.toString().trim() === '') {
          errosLinha.push('Setor obrigatório');
        }
        if (!linha.dataEntrada || linha.dataEntrada.toString().trim() === '') {
          errosLinha.push('Data entrada obrigatória');
        }
        
        // Validar preço entrada
        const precoEntrada = parseFloat(linha.precoEntrada);
        if (!linha.precoEntrada || isNaN(precoEntrada) || precoEntrada <= 0) {
        ...
        if (linha.precoTeto) {
          precoTeto = parseFloat(linha.precoTeto);
          if (isNaN(precoTeto)) {
        ...
        if (linha.precoTetoBDR) {
          precoTetoBDR = parseFloat(linha.precoTetoBDR);
        ...
        if (errosLinha.length > 0) {
          ...
          dados.push({
            ticker: linha.ticker.toString().trim().toUpperCase(),
            setor: linha.setor.toString().trim(),
            dataEntrada: linha.dataEntrada.toString().trim(),
