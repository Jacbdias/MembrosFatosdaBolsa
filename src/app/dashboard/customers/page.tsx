...
// 🔥 DETECÇÃO DE DISPOSITIVO - DUAS ESTRATÉGIAS SEPARADAS (IGUAL AO CÓDIGO 1)
...
// 🌐 DETECÇÃO ESPECÍFICA PARA APIs - IPAD SEMPRE MOBILE (ADICIONADO DO CÓDIGO 1)
...
async function buscarCotacoesParalelas(tickers: string[], isApiMobile: boolean): Promise<Map<string, any>> { // 🔥 MUDANÇA: era isMobile
  ...
  if (!isApiMobile) { // 🔥 MUDANÇA: era !isMobile
    ...
    try {
      ...
      const response = await fetch(`https://brapi.dev/api/quote/${tickers.join(',')}?token=${BRAPI_TOKEN}`, {
        ...
        headers: {
          'Accept': 'application/json',
          'User-Agent': 'MicroCaps-Desktop-Optimized'
        }
      });
  ...
  // Mobile: busca sequencial (ESTRATÉGIA MOBILE UNIVERSAL IGUAL AO CÓDIGO 1)
...
// 🚀 HOOK PRINCIPAL OTIMIZADO COM LOADING STATES GRANULARES - ADAPTADO PARA MICROCAPS (CORRIGIDO)
function useMicroCapsIntegradas() {
  ...
  const microCapsData = dados.microCaps || []; // 🔥 USANDO microCaps em vez de smallCaps
  ...
  const buscarDadosCompletos = React.useCallback(async () => {
    ...
    try {
      ...
      console.log('🚀 INICIANDO BUSCA STEP-BY-STEP ROBUSTA - MICROCAPS...');
      ...
      const proventosData = await buscarProventosAtivos(microCapsData);
      console.log('💰 Proventos obtidos:', proventosData.size, 'de', microCapsData.length);
  ...
  }, [microCapsData, isApiMobile, buscarProventosAtivos]); // 🔥 MUDANÇA: era isMobile

  // 🏆 USEEFFECT QUE SÓ EXECUTA QUANDO TODOS OS DADOS ESTÃO PRONTOS
  React.useEffect(() => {
    if (!todosOsDadosProntos || microCapsData.length === 0) return;
    ...
    console.log('📊 Dados disponíveis:', {
      ...
      ativos: microCapsData.length
    });

    const novasCotacoes: any = {};

    // 🎯 PROCESSAR TODOS OS ATIVOS COM TOTAL RETURN CORRETO
    const ativosFinais = microCapsData.map((ativo, index) => {
  ...
  }, [todosOsDadosProntos, cotacoesCompletas, dyCompletos, proventosCompletos, microCapsData]);
...
// 🎯 COMPONENTE PRINCIPAL OTIMIZADO - ADAPTADO PARA MICROCAPS
export default function MicroCapsPage() {
  ...
  // Separar ativos com memoização - SEM separação de encerrados para MicroCaps
