...
function calcularViesAutomatico(precoTeto: number | undefined, precoAtual: string): string {
  // 🎯 PARA ETFs INTERNACIONAIS: Se não há preço teto, sempre COMPRA
...
// 🚀 FUNÇÃO PARA BUSCAR DY 12 MESES VIA YAHOO FINANCE API (PADRONIZADA)
...
async function buscarCotacoesETFsParalelas(tickers: string[], isApiMobile: boolean): Promise<Map<string, any>> {
  const BRAPI_TOKEN = 'jJrMYVy9MATGEicx3GxBp8';
  ...
  if (!isApiMobile) {
    ...
    try {
      ...
      const response = await fetch(`https://brapi.dev/api/quote/${tickers.join(',')}?token=${BRAPI_TOKEN}&range=1d&interval=1d&fundamental=true`, {
        ...
        headers: {
          'Accept': 'application/json',
          'User-Agent': 'ETFsInternacionais-Desktop-Optimized'
        }
      });
  ...
  // 📱 MOBILE/iPad: busca individual com estratégias múltiplas (FUNCIONAL)
...
// 🚀 HOOK CORRIGIDO PARA BUSCAR COTAÇÕES DOS ETFS DO DATASTORE (ATUALIZADO PARA USAR isApiMobile)
function useETFsInternacionaisIntegradas() {
  ...
  const buscarCotacoesIntegradas = React.useCallback(async () => {
    try {
      ...
      console.log('🔥 BUSCANDO COTAÇÕES INTEGRADAS PARA ETFS INTERNACIONAIS - ESTRATÉGIA MOBILE/DESKTOP');
      ...
      const cotacoesMap = await buscarCotacoesETFsParalelas(tickers, isApiMobile); // 🔥 MUDANÇA: usar isApiMobile
      ...
      const ativosComCotacoes = await Promise.all(
        etfsInternacionaisData.map(async (ativo, index) => {
  ...
  }, [etfsInternacionaisData, isApiMobile]); // 🔥 MUDANÇA: adicionar isApiMobile na dependência
...
export default function ETFsInternacionaisPage() {
  const { dados } = useDataStore();
  const { ativosAtualizados, loading, isMobile } = useETFsInternacionaisIntegradas(); // ✅ isMobile do hook
  ...
  return (
    <div style={{ 
      ...
      <div style={{
        ...
        <div style={{
          ...
          <p style={{
            ...
            {loading ? 'Carregando...' : `${ativosAtualizados.length} ETFs`}
          </p>
        </div>
        ...
        {loading && ativosAtualizados.length === 0 ? (
          ...
          <>
            {isMobile ? (
              ...
              // 🖥️ DESKTOP: Tabela completa (sem coluna CATEGORIA)
      ...
      {/* Gráfico de Composição por Categoria Responsivo */}
      <div style={{
        backgroundColor: '#ffffff',
        borderRadius: '16px',
        border: '1px solid #e2e8f0',
