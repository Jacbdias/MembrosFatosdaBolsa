...
export async function GET() {
  try {
    ...
    if (!result?.regularMarketPrice) {
      return new Response(JSON.stringify({ error: 'Dados do IFIX não encontrados' }), { status: 500 });
    }
    ...
    const ifix = {
      ...
      fonte: 'BRAPI',
      nota: 'Dados via BRAPI'
    };

    return Response.json({ ifix });
  } catch (err) {
    return new Response(JSON.stringify({ error: 'Erro ao buscar IFIX', detail: String(err) }), {
      status: 500
    });
  }
}
