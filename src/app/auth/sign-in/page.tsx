...
export function SignInForm(): React.JSX.Element {
  ...
  const onSubmit = React.useCallback(
    async (values: Values): Promise<void> => {
      setIsPending(true);
      console.log('Tentando login com:', values); // DEBUG

      try {
        const { error } = await authClient.signInWithPassword(values);
        console.log('Resultado do signInWithPassword:', error); // DEBUG
        ...
        await checkSession?.();
        console.log('Sessão verificada, atualizando router'); // DEBUG
        router.refresh();
      } catch (err) {
        console.error('Erro inesperado no login:', err); // DEBUG
        setIsPending(false);
      }
    },
